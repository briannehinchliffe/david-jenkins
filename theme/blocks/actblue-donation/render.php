...
<div <?php echo $wrapper_attributes; ?> data-base-url="<?php echo esc_url( $actblue_url ); ?>">
	<div class="mb-8">
		<p class="text-sm font-medium text-primary mb-3">Select an amount:</p>
			...
			<?php foreach ( $preset_amounts as $amount ) : ?>
				<label class="cursor-pointer">
					<input
						...
						class="peer sr-only donation-radio"
						<?php checked( $amount, $default_amount ); ?>
					>
					<span class="flex items-center justify-center border rounded-lg px-3 py-2.5 text-sm font-semibold transition-all border-primary/15 text-primary bg-secondary hover:border-accent hover:text-accent peer-checked:border-accent peer-checked:bg-accent peer-checked:text-white peer-checked:hover:text-white peer-checked:hover:bg-red-700 peer-focus-visible:outline peer-focus-visible:outline-2 peer-focus-visible:outline-offset-2 peer-focus-visible:outline-accent">
		...
		<div class="mt-3 flex items-center gap-3">
			<span class="text-sm text-muted-foreground">Or enter a custom amount:</span>
			<div class="relative">
				...
				<input
					...
					step="1"
					name="custom_amount"
					class="custom-amount-input w-28 bg-secondary border border-primary/15 rounded-lg pl-7 pr-3 py-2 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/25 transition"
				>
			</div>
		</div>
	</div>
	...
	>
		...
		Donate <span class="display-amount">$<?php echo esc_html( $default_amount ); ?></span> via ActBlue
