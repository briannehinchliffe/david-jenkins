...
* Responsive fixed header partial (`header-content.php`) with dynamic admin bar offset, custom logo support, primary navigation menu, and mobile menu toggle state markup.
* Site footer partial (`footer-content.php`) featuring active widget sidebar support, footer menu navigation, contact details, campaign donation CTA button, copyright info, and official campaign disclaimer.
* Page template partials (`page.php` and `content-page.php`) integrated with template fallback functions and accessibility skip links.
* Extended component styling in `components.css` covering navigation hover states, admin bar offsets, and standard entry element boundaries.
* Custom block utilities in `utilities.css`, including:
